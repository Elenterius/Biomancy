...
import com.github.elenterius.blightlings.capabilities.SpecialSingleItemStackHandler;
...
public class ItemStorageItem extends BagItem implements IKeyListener {
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
		...
		tooltip.add(new StringTextComponent("Mode: ").mergeStyle(TextFormatting.GRAY)
				.append(new TranslationTextComponent("enum.blightlings.mode." + getMode(stack).toString().toLowerCase(Locale.ROOT)).mergeStyle(TextFormatting.AQUA)));
		tooltip.add(new TranslationTextComponent("tooltip.blightlings.press_button_to", ClientSetupHandler.ITEM_DEFAULT_KEY_BINDING.func_238171_j_().copyRaw().mergeStyle(TextFormatting.AQUA), new TranslationTextComponent("tooltip.blightlings.action_cycle")).mergeStyle(TextFormatting.DARK_GRAY));
	}
		...
		if (parentNBT.contains("Inventory")) {
			...
			if (!storedStack.isEmpty()) {
				...
				return new StringTextComponent("").append(displayName).appendString(" (")
						.append(new TranslationTextComponent("enum.blightlings.mode." + getMode(stack).toString().toLowerCase(Locale.ROOT))).appendString(", " + amount + "x ")
						.append(storedStack.getDisplayName()).appendString(")");
			}
		}

		return new StringTextComponent("").append(displayName).appendString(" (")
				.append(new TranslationTextComponent("enum.blightlings.mode." + getMode(stack).toString().toLowerCase(Locale.ROOT))).appendString(", Empty)");
	}
