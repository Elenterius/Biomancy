...
public class EntityStorageItem extends BagItem {
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
		...
		if (parentNBT.contains("Entity")) {
			...
			tooltip.add(new TranslationTextComponent("tooltip.biomancy.contains", new TranslationTextComponent(childNBT.getString("Name")).mergeStyle(TextFormatting.GRAY)));
		}
		...
		if (player != null) {
			...
			if (nbt.contains("Entity")) {
				...
				if (context.getWorld().isBlockModifiable(player, context.getPos()) && player.canPlayerEdit(context.getPos(), context.getFace(), stack)) {
					...
					ITextComponent textComponent = new TranslationTextComponent("msg.biomancy.failed_to_spawn", new TranslationTextComponent(wrapper.getString("Name"))).mergeStyle(TextFormatting.RED);
					player.sendStatusMessage(textComponent, true);
					return ActionResultType.FAIL;
				}
				ITextComponent textComponent = new TranslationTextComponent("msg.biomancy.not_allowed").mergeStyle(TextFormatting.RED);
				player.sendStatusMessage(textComponent, true);
				return ActionResultType.FAIL;
			}
		}
	...
	public ActionResultType itemInteractionForEntity(ItemStack stack, PlayerEntity playerIn, LivingEntity target, Hand hand) {
		...
		if (isValidTarget) {
			...
			if (!nbt.contains("Entity")) {
				...
				playerIn.sendStatusMessage(new TranslationTextComponent("msg.biomancy.already_full").mergeStyle(TextFormatting.RED), true);
			}
		}
