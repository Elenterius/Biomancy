...
public class ChromaSheepWoolLayer extends LayerRenderer<ChromaSheepEntity, SheepModel<ChromaSheepEntity>> {
	...
	public void render(MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn, ChromaSheepEntity entity, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
		if (!entity.isSheared() && !entity.isInvisible()) {
			...
			int colorTicks = entity.tickCount / 25 + entity.getId();
			float[] colorA = SheepEntity.getColorArray(DyeColor.byId(colorTicks % nColors));
			float[] colorB = SheepEntity.getColorArray(DyeColor.byId((colorTicks + 1) % nColors));
			float shift = ((entity.tickCount % 25) + partialTicks) / 25f;
			float r = colorA[0] * (1f - shift) + colorB[0] * shift;
			float g = colorA[1] * (1f - shift) + colorB[1] * shift;
			float b = colorA[2] * (1f - shift) + colorB[2] * shift;

			coloredCutoutModelCopyLayerRender(getParentModel(), model, TEXTURE, matrixStackIn, bufferIn, packedLightIn, entity, limbSwing, limbSwingAmount, ageInTicks, netHeadYaw, headPitch, partialTicks, r, g, b);
		}
	}
}
