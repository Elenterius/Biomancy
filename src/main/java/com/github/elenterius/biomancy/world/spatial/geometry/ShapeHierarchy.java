...
public class ShapeHierarchy<T extends Shape> {
	...
	protected void addShapeToSections(T shape) {
		AABB boundingBox = shape.getAABB();
		int minSectionX = SectionPos.blockToSectionCoord(Mth.floor(boundingBox.minX));
		int minSectionY = SectionPos.blockToSectionCoord(Mth.floor(boundingBox.minY));
		int minSectionZ = SectionPos.blockToSectionCoord(Mth.floor(boundingBox.minZ));
		int maxSectionX = SectionPos.blockToSectionCoord(Mth.floor(boundingBox.maxX));
		int maxSectionY = SectionPos.blockToSectionCoord(Mth.floor(boundingBox.maxY));
		int maxSectionZ = SectionPos.blockToSectionCoord(Mth.floor(boundingBox.maxZ));
	...
	protected Set<T> getShapesInSection(double x, double y, double z) {
		int sectionX = SectionPos.blockToSectionCoord(Mth.floor(x));
		int sectionY = SectionPos.blockToSectionCoord(Mth.floor(y));
		int sectionZ = SectionPos.blockToSectionCoord(Mth.floor(z));
	...
	public boolean intersectsCuboid(double minX, double minY, double minZ, double maxX, double maxY, double maxZ) {
		...
		int sectionMinX = SectionPos.blockToSectionCoord(Mth.floor(Mth.clamp(minX, aabb.minX, aabb.maxX)));
		int sectionMinY = SectionPos.blockToSectionCoord(Mth.floor(Mth.clamp(minY, aabb.minY, aabb.maxY)));
		int sectionMinZ = SectionPos.blockToSectionCoord(Mth.floor(Mth.clamp(minZ, aabb.minZ, aabb.maxZ)));
		int sectionMaxX = SectionPos.blockToSectionCoord(Mth.floor(Mth.clamp(maxX, aabb.minX, aabb.maxX)));
		int sectionMaxY = SectionPos.blockToSectionCoord(Mth.floor(Mth.clamp(maxY, aabb.minY, aabb.maxY)));
		int sectionMaxZ = SectionPos.blockToSectionCoord(Mth.floor(Mth.clamp(maxZ, aabb.minZ, aabb.maxZ)));
