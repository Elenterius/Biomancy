	...
	public void onSacrifice(ServerLevel level) {
		...
		if (level.random.nextFloat() < sacrificeHandler.getSuccessChance()) {
			...
			if (sacrificeHandler.getSuccessChance() > -9999) {
				...
				level.players().forEach(player -> player.sendMessage(ComponentUtil.literal("How dare you do this... I am watching you!").withStyle(TextStyles.MAYKR_RUNES_RED), Util.NIL_UUID));
			}
		}
