...
public final class PrimordialEcosystem {
	...
	public static <T extends FleshBlob & PrimordialFleshkin> boolean placeMalignantBlocks(ServerLevel level, BlockPos pos, T fleshBlob) {
		...
		Random random = fleshBlob.getRandom();

		if (currentState.is(veinsBlock)) {
			if (veinsBlock.getSpreader().spreadAll(currentState, level, pos) > 0) {
				for (Direction subDirection : ArrayUtil.shuffleCopy(Direction.values(), random)) {
	...
	public static boolean isReplaceableLog(BlockState state) {
		return isReplaceable(state) && state.is(BlockTags.LOGS_THAT_BURN);
	}

	public static Random getRandomWithSeed(BlockPos pos) {
