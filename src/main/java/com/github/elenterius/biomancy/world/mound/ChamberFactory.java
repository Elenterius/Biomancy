...
public interface ChamberFactory {
	void create(double x, double y, double z, float chamberRadius, Random random, Consumer<MoundChamber> consumer);

	ChamberFactory ONE_SPHERE = (double x, double y, double z, float chamberRadius, Random random, Consumer<MoundChamber> consumer) -> consumer.accept(new MoundChamber(new SphereShape(x, y, z, chamberRadius)));

	ChamberFactory EIGHT_SMALL_ELLIPSOIDS = (double x, double y, double z, float chamberRadius, Random random, Consumer<MoundChamber> consumer) -> {
	...
	ChamberFactory ONE_BIG_FOUR_SMALL_ELLIPSOIDS = (double x, double y, double z, float chamberRadius, Random random, Consumer<MoundChamber> consumer) -> {
		float halfR = chamberRadius / 2;
		float quarterR = halfR / 2;
		float p = chamberRadius / 3.8f; // radius / 4.25f
	...
	ChamberFactory FOUR_SMALL_ONE_BIG_ELLIPSOIDS = (double x, double y, double z, float chamberRadius, Random random, Consumer<MoundChamber> consumer) -> {
		float halfR = chamberRadius / 2;
		float quarterR = halfR / 2;
		float p = chamberRadius / 3.8f; // radius / 4.25f
	...
	ChamberFactory TWO_BIG_ELLIPSOIDS = (double x, double y, double z, float chamberRadius, Random random, Consumer<MoundChamber> consumer) -> {
		float halfR = chamberRadius / 2;
		float quarterR = halfR / 2;
		float p = chamberRadius / 3.8f; // radius / 4.25f
	...
	ChamberFactory SPECIAL_CRADLE = (double x, double y, double z, float chamberRadius, Random random, Consumer<MoundChamber> consumer) -> {
		float halfR = chamberRadius / 2;
		float quarterR = halfR / 2;
		float p = chamberRadius / 3.8f; // radius / 4.25f
	...
	ChamberFactory RANDOM_DEFAULT = (double x, double y, double z, float chamberRadius, Random random, Consumer<MoundChamber> consumer) -> {
