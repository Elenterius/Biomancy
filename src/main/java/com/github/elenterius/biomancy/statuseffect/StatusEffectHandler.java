...
public final class StatusEffectHandler {
	...
	public static void onEffectExpiry(final MobEffectEvent.Expired event) {
		...
		if (effectInstance != null && effectInstance.getEffect() == ModMobEffects.FRENZY.get()) {
			event.getEntity().addEffect(new MobEffectInstance(MobEffects.CONFUSION, 20 * 3, 1));
			event.getEntity().addEffect(new MobEffectInstance(ModMobEffects.WITHDRAWAL.get(), FrenzySerum.DEFAULT_DURATION_TICKS, FrenzySerum.AMPLIFIER));
		}
	}
