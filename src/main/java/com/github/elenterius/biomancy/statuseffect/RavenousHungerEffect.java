...
//		ModTags.Items.RAW_MEATS.getAllElements().stream().map(ItemStack::new).collect(Collectors.toList());
		return ImmutableList.of(new ItemStack(Items.MELON_SLICE), new ItemStack(Items.HONEY_BOTTLE), new ItemStack(ModItems.FLESH_MELON_SLICE.get()), new ItemStack(ModItems.COOKED_FLESH_MELON_SLICE.get()));
	}
}
