...
public abstract class ModEntityTypes
{
    ...
    public static void registerBiomeSpawns(final BiomeLoadingEvent event) {
        if (event.getCategory() == Biome.Category.SWAMP || event.getName() == Biomes.DARK_FOREST_HILLS.getRegistryName() || event.getName() == Biomes.DARK_FOREST.getRegistryName()) {
            event.getSpawns().getSpawner(EntityClassification.MONSTER).add(new MobSpawnInfo.Spawners(BLOBLING.get(), 100, 4, 8));
            event.getSpawns().getSpawner(EntityClassification.MONSTER).add(new MobSpawnInfo.Spawners(BROOD_MOTHER.get(), 100, 1, 2));
        }
    }
}
