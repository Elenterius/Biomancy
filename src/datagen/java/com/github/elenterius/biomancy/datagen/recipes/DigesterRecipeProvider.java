...
public class DigesterRecipeProvider extends RecipeProvider {
	...
	private void buildFromOrganicRecipes(Consumer<FinishedRecipe> consumer) {
		...
		simpleDigesterRecipe(ModItems.NUTRIENT_PASTE.get(), 2, Items.MOSS_BLOCK).setCraftingCost(3).modifyCraftingTime(x -> x + 20).save(consumer);

		simpleDigesterRecipe(ModItems.NUTRIENT_PASTE.get(), 2, Items.NETHER_WART).modifyCraftingTime(x -> x - 40).save(consumer);
		simpleDigesterRecipe(ModItems.NUTRIENT_PASTE.get(), 2, Items.CACTUS).setCraftingCost(3).save(consumer);
		...
		simpleDigesterRecipe(ModItems.NUTRIENT_PASTE.get(), 4, Items.BIG_DRIPLEAF).setCraftingCost(3).save(consumer);
		...
		simpleDigesterRecipe(ModItems.NUTRIENT_PASTE.get(), 4, Items.SHROOMLIGHT).setCraftingCost(3).save(consumer);

		simpleDigesterRecipe(ModItems.NUTRIENT_PASTE.get(), Foods.DRIED_KELP.getNutrition(), Items.KELP).setCraftingCost(3).modifyCraftingTime(x -> x + 35).save(consumer);
		...
		simpleDigesterRecipe(ModItems.NUTRIENT_PASTE.get(), 9 * wheatNutrition, Items.HAY_BLOCK).modifyCraftingTime(x -> x + 40).save(consumer);

		simpleDigesterRecipe(ModItems.NUTRIENT_PASTE.get(), 4, Items.COCOA_BEANS).setCraftingCost(3).modifyCraftingTime(x -> x + 60).save(consumer);

		int eggNutrition = 1;
		simpleDigesterRecipe(ModItems.NUTRIENT_PASTE.get(), eggNutrition, Items.EGG).modifyCraftingTime(x -> x + 20).save(consumer);
		simpleDigesterRecipe(ModItems.NUTRIENT_PASTE.get(), 2, Items.TURTLE_EGG).save(consumer);
		simpleDigesterRecipe(ModItems.NUTRIENT_PASTE.get(), 6, Items.SNIFFER_EGG).setCraftingCost(3).save(consumer);
		...
		simpleDigesterRecipe(ModItems.NUTRIENT_PASTE.get(), mushroomNutrition * 2, Items.RED_MUSHROOM_BLOCK).setCraftingCost(3).save(consumer);
		simpleDigesterRecipe(ModItems.NUTRIENT_PASTE.get(), mushroomNutrition * 2, Items.BROWN_MUSHROOM_BLOCK).setCraftingCost(3).save(consumer);
		simpleDigesterRecipe(ModItems.NUTRIENT_PASTE.get(), 2, Items.MUSHROOM_STEM).setCraftingCost(3).save(consumer);
		...
		simpleDigesterRecipe(ModItems.NUTRIENT_PASTE.get(), 7 * Foods.MELON_SLICE.getNutrition(), Items.MELON).setCraftingCost(3).save(consumer);

		int pumpkinNutrition = Foods.PUMPKIN_PIE.getNutrition() - sugarNutrition - eggNutrition;
		simpleDigesterRecipe(ModItems.NUTRIENT_PASTE.get(), pumpkinNutrition, Items.PUMPKIN).setCraftingCost(3).save(consumer);
	...
	private void buildAlexsCavesRecipes(Consumer<FinishedRecipe> consumer) {
		...
		createAlexsCavesRecipe(4, ACBlockRegistry.SUBTERRANODON_EGG.get()).setCraftingCost(3).save(consumer);
		createAlexsCavesRecipe(4, ACBlockRegistry.VALLUMRAPTOR_EGG.get()).setCraftingCost(3).save(consumer);
