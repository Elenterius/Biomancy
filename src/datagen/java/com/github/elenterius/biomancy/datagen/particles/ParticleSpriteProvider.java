...
public abstract class ParticleSpriteProvider implements DataProvider {
	...
	@Override
	public void run(HashCache cachedOutput) throws IOException {
		particles.clear();
		registerParticles();
		validate();
	...
	private void generate(final HashCache cache) {
		for (Map.Entry<ResourceLocation, ParticleSprite> entry : particles.entrySet()) {
			...
			try {
				DataProvider.save(GSON, cache, particleSprite.toJson(), getPath(particleId));
			}
			catch (IOException e) {
