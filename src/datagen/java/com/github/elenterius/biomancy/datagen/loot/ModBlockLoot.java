...
public class ModBlockLoot extends BlockLoot {
	...
	protected static LootTable.Builder createFleshSpikeTable(FleshSpikeBlock block) {
		...
		IntStream.range(FleshSpikeBlock.SPIKES.getMin() + 1, FleshSpikeBlock.SPIKES.getMax() + 1)
				.mapToObj(spikes ->
						SetItemCountFunction.setCount(ConstantValue.exactly(spikes))
								.when(
										LootItemBlockStatePropertyCondition
												.hasBlockStateProperties(block)
												.setProperties(StatePropertiesPredicate.Builder.properties().hasProperty(FleshSpikeBlock.SPIKES.get(), spikes))
								)
				)
		...
		return LootTable.lootTable().withPool(LootPool.lootPool()
				...
				.add(applyExplosionDecay(block, builder)));
	}
	...
	protected void addTables() {
		...
		add(ModBlocks.MALIGNANT_FLESH_VEINS.get(), block -> createFleshVeinsDrops(block, HAS_SHEARS_OR_SILK_TOUCH));
